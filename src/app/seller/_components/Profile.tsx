...
import { BookOpenIcon, BuildingLibraryIcon, CreditCardIcon, IdentificationIcon, MegaphoneIcon, PencilIcon, PencilSquareIcon, TrashIcon } from '@heroicons/react/24/outline'
...
export default function Profile({seller}:{seller:ProfileSellerProps}) {
    ...
    const uploadAboutMe = async()=>{
         setLoading(true)
         if(!dataSeller) return  toast.error("Error DataSeller is null", { id: "1" });;
         const res = await UpdateSeller(dataSeller)
         if(res.message !== "Success" ) return toast.error("Error occured during upload", { id: "1" });;
         setLoading(false);
         setShowForm(false)
    }
  ...
  return (
    <div className=' flex flex-col px-6 py-3'>
        <div id="profile-background" className=' min-w-full  z-0 p-0 flex justify-center md:justify-start md:pl-8'>
            <Image src={dataSeller?.image || "/images/bg1.jpg"} width={1000} height={1000} alt='Profile-seller' 
            ...
            <div className=' md:pl-[6rem] pt-[5rem]  md:pt-0 flex flex-col lg:flex-row justify-between gap-y-3'>
                ...
                <div id="button-edit-delete" className=' z-10  flex flex-col sm:flex-row gap-3 items-center justify-center md:justify-start'>
                    <button className="btn bg-slate-800 hover:bg-slate-700 text-white flex items-center gap-3"
                      onClick={() => (document.getElementById(`Edit_profile_${dataSeller?.id}`) as HTMLDialogElement).showModal()}
                    >
                        <PencilSquareIcon  className="h-4 w-4" />
                        Edit profile
                    </button>
                    <DialogEditSeller name_id={`Edit_profile_${dataSeller?.id}`} banks={banks} data_seller={seller} />
                    <button className="btn bg-white hover:bg-gray-100 border border-slate-500  flex items-center gap-3"
                     onClick={() => (document.getElementById(`delete_profile_${dataSeller?.id}`) as HTMLDialogElement).showModal()}
                    >
                        <TrashIcon  className="h-4 w-4" />
                            Delete
                    </button>
                    <DialogDelete name_id={`delete_profile_${dataSeller?.id}`} title='Account' setDeleted={setConfirmDelete}/>
                </div>
            </div>
        </div>
        <div id="Description" className=' mt-[-1.5rem] bg-white  min-h-[10rem] pt-[3rem] px-8 rounded-2xl pb- shadow-sm'>
            <h1 className=' flex items-center pb-2 gap-2 border-b border-gray-100 uppercase text-sm font-semibold text-gray-600'> 
                ...
                <div className=' flex items-center gap-3  lowercase font-thin text-gray-500 border-l border-gray-200 ml-6 pl-6' >
                    <button type='submit' className={`${(dataSeller?.about_me) && showForm?"block":"hidden"} flex items-center px-3 py-1 no-underline bg-sky-500 text-white rounded-lg`}
                    ...
                    <button className='flex items-center underline gap-1 ' onClick={()=>setShowForm((prev)=>!prev)}>
                        <PencilIcon className='w-3 h-3'/>{!showForm ?"write":"cancel"}
                    </button>
                </div>
            </h1>
            {showForm ? (
                <div className=" mt-4">
                <Textarea label="message" name='about_me' value={dataSeller?.about_me} color='blue' 
                    onChange={(e) => {
                        setDataSeller((prevValue) => ({ ...prevValue, about_me: e.target.value } as ProfileSellerProps));
                    }}
                />
                </div>
            ):(
        ...
        <div id="content" className=' pt-[2rem] pb-10 flex flex-col gap-3 md:flex-row  mt-3 bg-white rounded-2xl shadow-sm p-4'>
            <div id="detail-account" className=' flex flex-col border border-stone-200 shadow-md flex-1  pb-4 gap-y-4 rounded-xl'>
                ...
                <div className=' flex flex-col px-2 sm:px-4  gap-y-4'>
                    <div className=' flex flex-col gap-x-2 '>
                        ...
                        <div className='  bg-stone-50 border border-gray-100 rounded-lg py-1 px-3 text-sm '>{dataSeller?.full_name}</div>
                    </div>
                    <div className=' flex flex-col  gap-x-2 '>
                        <label className=' text-sm font-normal text-gray-600'>  Account name:</label>
                        <div className=' bg-gray-50 border border-gray-100  rounded-lg py-1 px-3 text-sm '>{ dataSeller?.user && dataSeller.user.length !==0 ? dataSeller.user[0].email:""}</div>
                    </div>
                    <div className=' flex flex-col  gap-x-2 '>
                    ...
                    <div className=' flex flex-col  gap-x-2 '>
                        <label className=' text-sm font-normal text-gray-600'> Phone:</label> text-sm
                        <div className=' bg-gray-50 border border-gray-100  rounded-lg py-1 px-3 '>{dataSeller?.phone}</div>
                    </div>
                    <div className=' flex flex-col  gap-x-2 '>
                        <label className=' text-sm font-normal text-gray-600'> Address:</label>
                        <div className=' bg-gray-50 border border-gray-100  rounded-lg py-1 px-3 text-sm '>{dataSeller?.address as string}</div>
                    </div>
                </div> 
            </div>
            <div className=' flex flex-1 flex-col gap-y-3 '>
                <div className='flex gap-3 flex-col sm:flex-row'>
                    <div className="stats shadow-md bg-white flex-1 border border-gray-200">
                        <div className="stat">
                            ...
                            <div className="stat-value flex items-center">{dataSeller?._count}
                ...
                <div id="bank-account" className=' flex flex-col border border-gray-200 shadow-md flex-1   rounded-xl'>
                    ...
                    <div className='flex flex-col px-2 sm:px-4   py-4 gap-y-4 '>
                        ...
                        <div className=' flex flex-col gap-x-2 '>
                            ...
                            <div className='  bg-stone-50 border border-gray-100 rounded-lg py-1 px-3 '>{dataSeller?.bank_name}</div>
                        </div>
                        <div className=' flex flex-col  gap-x-2 '>
                            <label className=' text-sm font-normal text-gray-600'>  Bank ID:</label>
                            <div className=' bg-gray-50 border border-gray-100  rounded-lg py-1 px-3 '>{dataSeller?.bank_id}</div>
                        </div>
                    </div>
                </div>
            </div>
