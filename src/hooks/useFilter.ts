...
export  function useDropdownFilter() {
    ...
    const handleFilter = (dataSheet: ExtendedSheet, key: string[]) => {
      const conditions = [
        dataSheet.type.toLowerCase().includes(key[0].toLowerCase()) || key[0] === "default",
        dataSheet.year.toLowerCase().includes(key[1].toLowerCase()) || key[1] === "default",
        dataSheet.semester.toLowerCase().includes(key[3].toLowerCase()) || key[3] === "default",
      ];
      return conditions.every(condition => condition);
    };
    

    const applyFiltersAndSort = (dataSheets: ExtendedSheet[],currentValues: string[]) => {
      ...
      if (currentValues[2] !== "default") {
        const filterSort = currentValues[2] === "min"
